...
**All nine variants now carry the full downstream** — pseudobulk KO-vs-WT DE, GO/GSEA and
identity GO, cluster markers, subtype map, composition and cell cycle. (An earlier draft of
this section said three did and the rest were *(labels only)*; that was true before the
sweep was completed and is no longer.) Selecting anything other than production shows a red
...
One caveat when comparing a number here against the rest of the app: the registry's
"production" arm is `cm_dims30_res0.2` and it yields **11** subclusters, while the shipped
res-0.2 clustering the other tabs are built on has **13** (CM0–CM12). Same dims, same
resolution, different partition — the Louvain label instability documented above. The
variant explorer's production arm is therefore a re-computation, not the shipped
partition.

One result survives the whole exercise, and **it is the opposite of what an earlier draft
of this section concluded.** With all nine variants now tested, the complete set of genes
significantly KO-up in *every* testable subcluster is, in each variant:

| variant | universally KO-up |
|---|---|
| dims10 res 0.1 / 0.2 | Ddx3y, Eif2s3y, **Gabbr2**, Kdm5d, **Tcf4**, Uty |
| dims10 res 0.3 | Ddx3y, Eif2s3y, **Gabbr2**, **Tcf4**, Uty |
| dims30 res 0.1 / 0.2, dims50 res 0.1 | Ddx3y, Eif2s3y, **Gabbr2**, **Tcf4** |
| dims30 res 0.3, dims50 res 0.2 / 0.3 | **Gabbr2**, **Tcf4** |

In all nine clusterings that set is Gabbr2, Tcf4 and **Y-chromosome genes, and nothing
else**; at the finest cuts it is exactly `{Gabbr2, Tcf4}`. The earlier draft read Gabbr2's
clustering-independence as evidence that it is a safe, whole-compartment effect. That
inference does not hold: `Ddx3y` and `Uty` have the identical property, and they are
sex markers. Genotype is perfectly confounded with sex in this design, so a gene whose only
distinction is that it moves everywhere is behaving like an animal-level difference, not
like biology — the same reasoning this project already used to reject the `mt-` block
(chapter 11, confound 5). Gabbr2 and Tcf4 should be treated as unresolved by this dataset,
not as its most robust finding.
