...
**The mundane reason there is no DE**, separately: the pseudobulk contrast aggregates cells
into one sample per library × lane, needs 20 cells per sample and then two KO and two WT
samples to estimate dispersion. Spread over eight possible samples, 99 cells clears the bar
in **one WT sample and no KO samples**, so there is no contrast to fit. It is recorded as
`skipped_too_few_or_unbalanced` rather than silently absent, which is how the gap was found.
